...
// Add Zen Tabs styles
if (theme_get_setting('destinationjump_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'destinationjump') .'/css/tabs.css');
}

function destinationjump_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . destinationjump_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = destinationjump_id_safe('page-'. $path);
    $vars['classes_array'][] = destinationjump_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('destinationjump_ie_enabled')) {
    $destinationjump_ie_enabled_versions = theme_get_setting('destinationjump_ie_enabled_versions');
    if (in_array('ie8', $destinationjump_ie_enabled_versions, TRUE)) {
    ...
    if (in_array('ie9', $destinationjump_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/css/ie9.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 9', '!IE' => FALSE), 'preprocess' => FALSE));
    }
    if (in_array('ie10', $destinationjump_ie_enabled_versions, TRUE)) {
...
function destinationjump_preprocess_page(&$vars, $hook) {
...
function destinationjump_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from destinationjump_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function destinationjump_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function destinationjump_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('destinationjump_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('destinationjump_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('destinationjump_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('destinationjump_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('destinationjump_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function destinationjump_id_safe($string) {
 ...
 *
 */
function destinationjump_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = destinationjump_id_safe($element['#title']);
 ...
 */
function destinationjump_preprocess_menu_local_task(&$variables) {
 ...
 */
function destinationjump_menu_local_tasks(&$variables) {
  $output = '';
